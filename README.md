...
- **n = 1 validation event.** The method is validated as a *ranker* on one fire (Thomas / Montecito). Transferability to other ranges, rain regimes, or fire types is unestablished; a second documented event is needed before any transferability claim.
...
- **dNBR (the production default) is triage-validated, not exact-rank-validated.** All exact-rank validation is on SBS; on the one fire where dNBR was swapped in, it matched SBS on rank-AUC but missed the pre-registered top-1 criterion. Any dNBR top-of-list order is provisional at n = 1.
...
- **The incised tier is unvalidated on its own terrain class.** It is an exploratory, disclaimed source-susceptibility triage — not the validated product. Where dissected terrain is uniformly steep, `mean_slope` may stop discriminating and the `intensity` ordering can degenerate toward a burn-severity map; this is an acknowledged open limitation carried verbatim on every incised artifact.
