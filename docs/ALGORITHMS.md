...
| `MASTER_KNOWN_KM2` | 39.19 km² | [config.py:61](../src/config.py#L61) | **Stale Week-0 reference** (see note below); PASS/ABORT bands built around it |
| `MASTER_PASS_LO/HI` | 33.3 / 45.1 km² | [config.py:62](../src/config.py#L62) | Master-outlet PASS band (±15% of 39.19); the live 44.7273 falls inside |
| `MASTER_ORDER_LO/HI` | 20.0 / 80.0 km² | [config.py:63](../src/config.py#L63) | Outside this -> ABORT (order-of-magnitude anti-0 km² guard) |
...
> **On `MASTER_KNOWN_KM2 = 39.19`.** This constant is the Week-0 documented master-outlet area, retained as the reference the gate prints and the value the PASS/ABORT bands are centered on. The **reconstructed** master outlet is **44.7273 km²** ([test_behavior_lock.py:94](../tests/test_behavior_lock.py#L94)), which still lands inside the wide PASS band (33.3-45.1), so the guard passes. Treat 44.7273 as the current truth and 39.19 as a superseded reference that the config has not been re-baselined to. This mirrors how a stale constant left in a doc is handled elsewhere in this project: the live behavior wins.
