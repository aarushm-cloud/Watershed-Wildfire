...
**Date:** July 6, 2026 · reconciled to the live tree 2026-07-11 · reconciled again 2026-07-20 (A39, §9) · **reconciled again 2026-07-21 (A37 CONUS coverage; auto-acquire subsystem, §7/§11)**
...
This document reflects the **live code tree**. **Reconciled 2026-07-11:** since the 2026-07-06 draft, three sections advanced in the tree and are updated below — the dNBR arm is now **wired** into production (A34, §6–§7), the coastal-slope nodata-ring drop **shipped** (A33 + the F4 slope-coverage flag, §4), and the pysheds flow model gained an independent **pyflwdir cross-check** (CF-11, §3). Where the code and an existing `.md` disagree on a number, the code wins and the discrepancy is flagged inline. The repo carries no separate architecture or science-reference doc — both are canonical in the vault — so this doc cites source lines directly.
...
**Reconciled 2026-07-28 (A41 gating fixes — code changed, unlike the entries below):** the 2026-07-27 stress test found two selection-time gating defects, fixed 2026-07-28 under the proposed principle **A41: the selector never proposes a pair the creator refuses** (PROPOSED, ratification owed). (1) S2 members below the frozen `S2_MIN_BASELINE = 4.0` are now dropped at search, before grouping ([scene_select.py:390](../autoacquire/scene_select.py#L390)) — previously a pre-2022 fire could receive a graded scorecard the creator aborted on *after* human approval. The constant moved verbatim to [scene_select.py:75](../autoacquire/scene_select.py#L75) (single source; the creator imports it — value unchanged, pre-reg D). (2) An S2 tile group spanning two UTM zones (MGRS overlap at a zone boundary — possible even when the *fire* is single-zone) is now an audit-trail **rejection** inside `select()` ([`_reject_zone_spanning`](../autoacquire/scene_select.py#L520)) instead of an escaping `GateAbort`, so the documented Landsat pair-level fallback is actually reachable (measured pre-fix: Montecito/Trout/Buck/Black all refused with 10–30 clean single-zone Landsat candidates available; 108°W bisects the Gila). A pair-level zone check ([`_pair_zone_ok`](../autoacquire/scene_select.py#L536)) mirrors the creator's `_zones` guard for the chosen pair AND swap alternatives. The creator's asserts remain as backstops; no frozen threshold moved; §7/§11 updated below.

**Reconciled 2026-08-08 (A40 ordering — doc-only):** §1 and §9 below previously still described the A39 ordering (`intensity` as the incised headline); corrected to the shipped **A40** behavior — the frozen `score`/`rank` is the headline on every tier and `intensity`/`intensity_rank` are companion columns, no re-sort ([outputs.py:216-221](../src/outputs.py#L216-L221)). Stale anchors updated: `add_intensity_rank` lives at [score.py:53-61](../src/score.py#L53-L61), called at [pipeline.py:471-473](../src/pipeline.py#L471-L473); `INCISED_FRAMING` at [outputs.py:123-139](../src/outputs.py#L123-L139).

**Reconciled again 2026-07-21 (A37 + auto-acquire):** two things that shipped in the code but never reached this doc are now written up. (1) **A37** widened `ALLOWED_UTM_ZONES` from the two onboarded fires to the whole CONUS coverage (UTM 10N–19N / EPSG 32610–32619): the metric-CRS allowlist is now a *coverage bound*, not an onboarding gate, and the per-area science gates behind it (the A27/A39 terrain router, the `CONTOUR_M` range guard, the dNBR coverage fail-loud) decide whether a ranking is produced (§2, §11). (2) The **auto-acquire dNBR subsystem** (`autoacquire/`) — deterministic scene-pair selection plus the NBR band math that produces the dNBR raster §7's ingest arm consumes — is documented in a new front-half of §7, with its frozen constants added to §11. This is a documentation-only reconciliation: no code changed, and the frozen fence and the range-front behavior lock are untouched.
...
7. [The dNBR burn-source arm: acquire, create, ingest (deep)](#7-the-dnbr-burn-source-arm-acquire-create-ingest-deep)
...
dNBR ──────> ingest.ingest_dnbr_both_arms (both arms; incised: subbasins.filter_burned_steep
   ...
   pipeline.mean_slope_tan(dem_raw) ──────────────────────── slope ──────┤
                    ...
                    area-independent) -> companion columns only; headline stays score (A40)
                                                                          │
...
All distance math is metric. `_assert_metric_crs` ([grids.py:23-37](../src/grids.py#L23-L37)) fails loud unless a layer's CRS is in the allowlist `ALLOWED_UTM_ZONES` ([config.py:63](../src/config.py#L63)). **A37 widened this from the two onboarded fires (`{32611, 32613}`) to the whole CONUS coverage — `set(range(32610, 32620))`, i.e. UTM 10N–19N / EPSG 32610–32619.** The allowlist is now a *coverage bound*, not an onboarding gate: any contiguous-US bbox clears this guard, and the per-area science gates behind it (the A27/A39 terrain router, the `CONTOUR_M` range guard, the dNBR coverage fail-loud) decide whether a ranking is produced — "correct areas only" lives there, not here. Any geographic CRS (e.g. EPSG:4326) or an out-of-CONUS zone (Alaska/Hawaii/non-US) still aborts rather than silently computing distances in degrees. Coverage is a separate axis from validity: only Montecito (32611) is validated; every other covered zone is covered-but-unvalidated, carried by the screening + "dNBR unvalidated for ranking" framing on every artifact (§11, A37).
...
## 7. The dNBR burn-source arm: acquire, create, ingest (deep)

**Files:** acquisition [autoacquire/scene_select.py](../autoacquire/scene_select.py) · creation [autoacquire/dnbr_create.py](../autoacquire/dnbr_create.py) · wiring [autoacquire/autoacquire_run.py](../autoacquire/autoacquire_run.py) · ingest [ingest.py:167-298](../src/ingest.py#L167-L298)
**Constants:** `DNBR_BIN_EDGES`, `DNBR_CLAMP`, `DNBR_FLOOR` at [config.py:32-38](../src/config.py#L32-L38); auto-acquire constants at [scene_select.py:50-86](../autoacquire/scene_select.py#L50-L86) and [dnbr_create.py:60-66](../autoacquire/dnbr_create.py#L60-L66) (see §11)
**Status:** implemented, unit-tested, and wired end-to-end (A34).

The dNBR arm has three parts, walked here in order:

1. **Acquire** a clean pre/post satellite scene pair (`autoacquire/scene_select.py`) — a deterministic selector over public Sentinel-2 / Landsat catalogs, gated by a human before anything is built.
2. **Create** the dNBR raster from that pair (`autoacquire/dnbr_create.py`) — the NBR band math, per-sensor surface-reflectance scaling, and cloud masking.
3. **Ingest** the native dNBR onto the DEM grid and bin/score it through the two arms (`ingest.py`) — the part documented here before, unchanged.

Parts 1–2 are the auto-acquire subsystem (the app's "Generate from dates" mode and the `autoacquire/` CLI); they are **skipped when the user uploads a dNBR they already have**, which lands directly at Part 3. There is **no LLM anywhere in this path** — it is deterministic optimization plus a human approval gate. Every value in Parts 1–2 is frozen by the ratified auto-acquire pre-registration (2026-07-17); a value that "looks off" is a finding, never an in-place edit.

### 7a. Acquire: deterministic scene-pair selection

**File:** [autoacquire/scene_select.py](../autoacquire/scene_select.py) — `select()` at [scene_select.py:483-570](../autoacquire/scene_select.py#L483-L570)

The selector turns a drawn bbox + ignition/containment dates into a recommended clean pre/post pair (plus ranked alternatives and an audit trail). Sentinel-2 is tried first; Landsat 8/9 is a **pair-level fallback, never mixed within a pair** (A2/A3). Five deterministic stages:

**Timing windows** (`derive_windows`, [scene_select.py:99-124](../autoacquire/scene_select.py#L99-L124)). `pre = [ignition − 90 d, ignition)` (`PRE_WINDOW_DAYS = 90`), strictly before ignition; `post = [containment, containment + greenup_days]`, first clean scene wins (the BAER initial-assessment convention). The post window is bounded by a **green-up ceiling** (`GREENUP_DEFAULT_DAYS = 90`, operator-extendable to a hard `GREENUP_MAX_DAYS = 180`) so a regrown post-fire scene can't wash the burn signal out.

**Coarse filter** (`coarse_filter`, [scene_select.py:132-165](../autoacquire/scene_select.py#L132-L165)) — metadata only, no pixels. Drops out-of-window dates, partial-AOI footprints (`footprint.covers(aoi)`), and whole-tile cloud over `TILE_CLOUD_MAX_PCT = 80.0%` (STAC `eo:cloud_cover`, strict `>`). Explicitly coarse and **never the decisive gate** — the "Elephant lesson" was a ~5%-cloud tile that was ~99.6% clouded over the actual fire.

**Decisive box gate** (`pair_metrics` / `passes_box_gate`, [scene_select.py:191-223](../autoacquire/scene_select.py#L191-L223)) — actual per-pixel cloud masks over the drawn box. The decisive number is the **intersection** valid fraction `pre_valid & post_valid` (two scenes each 90% clean in *different* places can still fail together), never each scene alone and never averaged. It must clear `BOX_GATE_FLOOR = 0.50`. That 0.50 is **derived, not published**: from the pipeline's own frozen per-basin `DNBR_NODATA_FAILLOUD_FRAC = 0.20` (a flowed basin needs ≥ 80% valid), stated as a derivation rather than dressed as a literature value. Per-pixel validity uses the same mask sets as creation: S2 `SCL ∈ {0,1,3,6,8,9,10,11}` → bad; Landsat `QA_PIXEL` fill bit 0 + cloud bits 1–4 → bad.

**Rubric verdict** (`rubric_verdict`, [scene_select.py:231-268](../autoacquire/scene_select.py#L231-L268)) — a Good / OK / Marginal / Below-bar scorecard on two axes: the pair-valid fraction (bands `RUBRIC_GOOD_PAIR = 0.90` / `RUBRIC_OK_PAIR = 0.75`) and the worst per-scene cloud-over-AOI (`RUBRIC_GOOD_SCENE_CLOUD = 0.05` / `RUBRIC_OK_SCENE_CLOUD = 0.15`). The verdict is the **worse** of the two axes, never blended, so identical metrics always produce the identical verdict and prose.

**Human approval** — `select()` proposes and scores; it **builds nothing**. The recommendation package carries the pair, the scorecard, AOI-clipped true-color previews (`render_rgb_preview` — display-only 2–98% stretch, never fed to any metric), and pre-vetted swap alternatives (each re-run through the same gate via `evaluate_pair`). A person approves before any dNBR is computed (the app captures it with a button; the CLI with `--approve`).

**Honest failure states** (`_failure_state`, [scene_select.py:573-613](../autoacquire/scene_select.py#L573-L613)) — never a fabricated pair: `no_pre_scene` (no clean pre within 90 d), `window_closed` (green-up ceiling passed with no clean post), or `waiting` (Mode B: no usable post-fire scene yet, with a next-overpass ETA and the caveat that an overpass is not a guaranteed clear view). Infrastructure failures (STAC unreachable, grid mismatch, an unreadable mask asset) raise `GateAbort`. Same-day adjacent tiles of one overpass are merged into one candidate (`group_candidates`) so a two-tile fire is not false-rejected.

**Selection-time eligibility (A41 fixes, 2026-07-28)** — the selector enforces, at selection time, the two constraints the creator would otherwise refuse *after* human approval. S2 members below the frozen baseline floor are dropped at search, before grouping ([scene_select.py:390](../autoacquire/scene_select.py#L390)), so a stale `_0_L2A` twin cannot poison a group whose `_1_L2A` reprocessing is usable, and a fire with no eligible S2 falls through to the Landsat arm. A tile group spanning two UTM zones — MGRS grids overlap at zone boundaries, so this happens even when the fire itself is single-zone — is rejected into the audit trail ([`_reject_zone_spanning`](../autoacquire/scene_select.py#L520)), never an escaping abort, and a cross-zone (pre, post) combination is skipped for the chosen pair and for swap alternatives alike ([`_pair_zone_ok`](../autoacquire/scene_select.py#L536)). The creator's own baseline assert and `_zones` guard remain as defense-in-depth backstops; the mask-read cross-zone abort is unreachable from `select()` but retained for direct callers.

STAC sources: Sentinel-2 L2A via Earth Search on AWS (`earth-search.aws.element84.com`, collection `sentinel-2-l2a`); Landsat 8/9 Collection-2 L2 via Microsoft Planetary Computer (`landsat-c2-l2`, asset hrefs SAS-signed at read time).

### 7b. Create: NBR band math and the dNBR raster

**File:** [autoacquire/dnbr_create.py](../autoacquire/dnbr_create.py) — `create_dnbr()` at [dnbr_create.py:77-167](../autoacquire/dnbr_create.py#L77-L167)

An approved pair becomes a raw, native-grid dNBR GeoTIFF. The science is transcribed verbatim from the agency docs and the validated working scripts — never reconstructed:

```
NBR  = (NIR − SWIR) / (NIR + SWIR)          # dimensionless, on scaled surface reflectance
dNBR = NBR_pre − NBR_post                    # raw scale (~ −0.5 … +1.3), positive = burned; NEVER ×1000
```

Per-sensor band adapters ([dnbr_create.py:175-247](../autoacquire/dnbr_create.py#L175-L247)):

| Sensor | NIR | SWIR | Res. | Surface-reflectance scaling |
|---|---|---|---|---|
| Sentinel-2 L2A | B8A (`nir08`) | B12 (`swir22`) | 20 m | `(DN − 1000) / 10000` — BOA offset −1000; requires processing baseline ≥ 04.00 (`S2_MIN_BASELINE = 4.0`, single-sourced in `scene_select`; enforced at search since the A41 fixes, and still asserted numerically per member here as the backstop — older products never reach approval) |
| Landsat 8/9 C2 L2 | B5 (`nir08`) | B7 (`swir22`) | 30 m | `DN × 0.0000275 − 0.2` — the additive offset means a ratio on raw DN is wrong; scale before any ratio |

Cloud/quality masking is a **union of bad pixels → NoData** (a ratified divergence from the validation lineage's fill-bit-only handling): S2 `SCL ∈ {0,1,3,6,8,9,10,11}`; Landsat `QA_PIXEL` fill bit 0 + cloud bits 1–4. The final dNBR is clipped to a physical `|dNBR| ≤ DNBR_PHYS_CLIP = 2.0` (NBR ∈ [−1, 1]); NoData is `−9999`.

**No reprojection happens here** — the output stays on the scene's native grid, and pre/post must land on the *identical* native lattice or the creator aborts loud (`STOP, do NOT resample`). This is deliberate: the single resample in the whole pathway is the frozen both-arms ingest (§7c), so the burn signal is never double-resampled. Before returning, the artifact is passed through the pipeline's own `acquire.assert_raw_dnbr` input gate (99th-percentile `|dNBR| ≤ 2`, fill-sentinel screen), so a bad raster can never be handed onward. A quicklook PNG and a provenance JSON (bands, scaling, scene IDs, mask description, framing) ride along.

### For the Environmental Scientist (acquisition + creation)

BAER soil burn severity is field-validated but only exists for fires someone assessed. dNBR is a satellite "how much did the landscape change" index available for any fire for free — the natural input for the un-assessed fires this tool targets. For a user who cannot make one, the tool builds it: it finds a cloud-free satellite view from just before the fire and one just after, subtracts them to isolate the burn scar, and hands back a checkable package — the two scenes, a plain-English "this covers ~N% of your fire; worst cloud M%" scorecard, and a true-color preview — for a person to approve before anything is computed. The machine proposes; a human disposes. If the sky was never clear enough, or no post-fire image exists yet, it says so plainly instead of inventing a burn map.

### 7c. Ingest: bin and score the native dNBR (Arms A and B)

Whether the dNBR was auto-acquired (§7a–b) or uploaded by the user, it enters the pipeline here — dispatched through `ingest_dnbr_both_arms` and scored through **both arms**. Arm A (binned) is the headline ranking; Arm B (continuous) rides alongside with `rank_delta = |rankA − rankB|` as an honest uncertainty flag ([outputs.py:181](../src/outputs.py#L181), `write_dnbr_outputs`). `ingest_burn`'s A29 guard fires only on a **partial-SBS** raster (§6). Validation: the Montecito dNBR both-arms run reproduces the P2.3 swap-test oracle (rank-AUC 0.9722, both arms), and the dNBR ranking is carried as **triage-validated, not exact-rank-validated** (n = 1 — the pre-registered exact-#1 criterion missed by 1.03%); that n = 1 framing is stamped on every dNBR artifact.

The bin edges, clamp, floor, and the 5→4 collapse are pre-registered in [validation/reports/P2_PREREGISTRATION.md](../validation/reports/P2_PREREGISTRATION.md) and frozen: a value that "looks off" is a P2.3 finding, never a P2.2b edit.
...
`reproject_dnbr` ([ingest.py:167-196](../src/ingest.py#L167-L196)) snaps native dNBR onto the DEM grid via explicit `dst_transform`/`dst_shape` (never the dNBR scene's own grid), Arm A with `Resampling.nearest`, Arm B with `Resampling.bilinear`. **This is the one and only resample in the whole dNBR pathway** — the creator (§7b) deliberately does none, so the burn signal is resampled exactly once. Sensor note: the native dNBR arrives either from the auto-acquire path (§7a–b — Sentinel-2 20 m primary, Landsat 8/9 30 m fallback) or from a user upload; whichever it is, it is upsampled onto the 10 m DEM grid (2× for S2, 3× for Landsat) and carried raw-scale, never ×1000 (A21/A4). The frozen `montecito_dnbr` validation fixture is Landsat-8 30 m. `ingest_dnbr_both_arms` ([ingest.py:243-298](../src/ingest.py#L243-L298)) derives one shared valid footprint from Arm A's nearest reproject and applies it to both arms, so A and B are byte-identical in footprint by construction; `assert_aligned` runs on both before any thresholding, and a fail-loud guard rejects any non-finite/sentinel value inside the valid footprint. The known-answer tests pin every bin boundary (0.100 left-closed to class 2, 0.440 collapsing to class 3, and so on), the linear Arm-B transfer, the shared floor, and the NaN-routing defect.
...
`stage_2e_score` also enforces the **A32** empty-mask guard ([score.py:49-54](../src/score.py#L49-L54)): if a basin mask has zero cells, `mean_slope` would be `np.mean(slope[[]]) = nan` and poison the score, so it raises `GateAbort` instead. The rationale is premises-void, not sibling-symmetry: `mean_burn`/`burn_coverage_frac` return `0.0` on an empty basin because zero is a meaningful value there, but an empty **mask** can only mean `delineate`'s `MIN_BASIN_KM2` guarantee broke, i.e. the run's premises are unsound, so the whole run aborts. The guard ships with a synthetic trip-fixture ([tests/core/test_empty_mask_abort.py](../tests/core/test_empty_mask_abort.py)); it is unreachable on Montecito.
...
`assess_hypsometric_applicability` — the detector itself — is retained **byte-identical**. What A39 changed is what happens with its verdict: instead of writing `refusal.json` and stopping, `span_m > 50.0` now selects the WhiteboxTools sub-basin engine (below) in place of the pysheds canyon-mouth engine. `write_refusal` was **removed 2026-07-20 by owner ruling** — dead machinery after A39, with no live caller once terrain shape stopped refusing; the detector-level firewall remains locked by `test_a27_applicability` group D, and `build_refusal_message` ([outputs.py:38-61](../src/outputs.py#L38-L61)) is kept for its one live caller in that file's group C message tests. `run_pipeline`'s polymorphic return contract and `dispatch_result`/`app.py`'s view model still support a `status="refused"` result for any future non-terrain refusal trigger, but no production code builds one, no code path writes a `refusal.json`, and every precondition failure (below) raises `GateAbort` directly — do not describe `refusal.json` as a reachable output (the ranked writers now purge a stale one on write).
...
On incised output, `score.add_intensity_rank` ([score.py:53-61](../src/score.py#L53-L61), called at [pipeline.py:471-473](../src/pipeline.py#L471-L473)) adds `intensity = mean_burn × mean_slope` — the frozen score with the area exponent set to zero, which A39 explicitly binds as a use of the deferred **C1** area-dampening family, scoped to incised terrain only; promoting it to range-front output is forbidden by the decision. Rows stay in the frozen `score`/`rank` order — the same headline as a range-front fire (**A40**, amending A39 clause 4); `intensity`/`intensity_rank` ride along as companion columns, never dropped and never a re-sort, disclosed because the score's area term depends on the segmentation threshold in this mode and because `intensity` scored HIGHER on the one (range-front) confirmation case (AUC 0.887 vs 0.790). `outputs.write_dnbr_outputs` stamps `INCISED_FRAMING` (verbatim EXPLORATORY / UNVALIDATED language, [outputs.py:123-139](../src/outputs.py#L123-L139)) plus WhiteboxTools engine provenance (`engine`, `wbt_version`, `acc_threshold_cells`, `breach_dist_cells`) in place of the plain provenance a range-front artifact carries; the `incised=False` default keeps pre-A39 callers byte-identical.

> **Surviving limitation, not solved by A39.** A28 gave three reasons an incised ranking is meaningless. A39 answers the area argument (`intensity`) and the ranking-unit argument (confluence splits). Verbatim from the A39 decision record: "It does NOT rebut the second: that `mean_slope` may stop discriminating on uniformly steep dissected highland, in which case `intensity` degenerates toward a burn-severity map. This is an ACKNOWLEDGED OPEN LIMITATION carried verbatim in `INCISED_FRAMING`, not a solved problem." `INCISED_FRAMING` itself says: "KNOWN OPEN LIMITATION: where dissected terrain is uniformly steep, mean_slope may not discriminate between basins, in which case the `intensity` companion approaches a burn-severity ranking."
>
> **The supporting evidence is range-front evidence, not incised-terrain evidence.** The pre-ratification confirmation run at shipping parameters (`MIN_BASIN_KM2 = 0.1`) reproduces, on **Montecito** — verbatim from the A39 decision record: "88 basins, AUC(intensity) 0.887, AUC(size) 0.790, 10/10 of the top-10 intensity-ranked basins flowed" (22/25 of top-25). That is the validated range-front case re-run under the incised engine's segmentation parameters, and it rests on "6 independent flow events — effective n = 6": "consistency-with-known-outcome, not generalizable predictive skill." The first incised-terrain evidence is a separate, later, pre-registered concordance check against a USGS assessment; its result is out of scope for this document and is not cited here.
...
> **Numbers discipline: read before quoting AUC.** The live, reconstructed oracle is **AUC 0.9722 / 36 basins / 44.7273 km² master outlet**, frozen in [tests/core/test_behavior_lock.py:83,94,102](../tests/core/test_behavior_lock.py#L83). The Week-0 report's **0.987 / 39.19 km² / 32 basins** are the **original-AOI** figures and are **not bit-reproducible** (the original AOI is unrecoverable). Do not cite them as current. Some older narrative notes still repeat `0.987 / 39.19 / 32` as "canonical-locked"; that is the stale copy. The behavior lock and `P2_PREREGISTRATION.md` both explicitly anchor on the reconstructed 0.9722 / 36 / 44.7273 instead. This is the same "a superseded number lives in a doc" situation the reader should treat like any stale constant: the code and the lock win.
...
| `CANONICAL_CRS` | EPSG:32611 | [config.py:50](../src/config.py#L50) | Montecito/UTM-11N validation zone; per-fire default |
| `ALLOWED_UTM_ZONES` | `set(range(32610, 32620))` (CONUS 10N–19N) | [config.py:63](../src/config.py#L63) | Metric-CRS allowlist; **A37** widened it from `{32611, 32613}` to a CONUS coverage bound, out-of-CONUS fails loud (§2) |
| `CELL_M` | 10.0 m | [config.py:64](../src/config.py#L64) | DEM resolution; `dx = dy`; slope spacing; `CELL_AREA_KM2 = 1e-4` |
| `DIRMAP` | `(64,128,1,2,4,8,16,32)` | [config.py:84](../src/config.py#L84) | **Frozen fence.** pysheds D8 encoding, `(N,NE,E,SE,S,SW,W,NW)` |
| `D8_OFFSETS` | code -> `(drow,dcol)` map | [config.py:85-86](../src/config.py#L85-L86) | **Frozen fence.** Decodes `fdir` to the downstream step at outlet detection |
...
| `SUBBASIN_SLOPE_FLOOR_TAN` | 0.05 (~2.9°) | [config.py:94](../src/config.py#L94) | **Frozen fence (A39).** Drops degenerate flat incised sub-basins only; NOT result-blind (set after seeing output — see decision A39) |
...
| **Oracle: rank-AUC** | **0.9722** (175/180) | [test_behavior_lock.py:83](../tests/core/test_behavior_lock.py#L83) | Behavior-lock value; reconstructed, not the report's 0.987 |
| **Oracle: master outlet** | **44.7273 km²** (±0.5) | [test_behavior_lock.py:94](../tests/core/test_behavior_lock.py#L94) | FM-1 anti-0 km² linchpin; reconstructed, not 39.19 |
| **Oracle: basin count** | **36 basins** | [test_behavior_lock.py:102](../tests/core/test_behavior_lock.py#L102) | AOI-shift delta vs the report's 32 (a finding, not a bug) |
| Oracle: truth set | `{4,6,9,14,21,23}` (6 flowed) | [test_behavior_lock.py:88](../tests/core/test_behavior_lock.py#L88) | Ground-truth flowed basins; 6/6 in top tercile; #1 = basin 6 |

> **Master-outlet guard is SCALE-FREE (A38, supersedes T5).** The FM-1 anti-collapse check no longer bins the domain pour-point into PASS / FINDING / ABORT km² bands centered on `MASTER_KNOWN_KM2` — those flagged on absolute size (calibrated to Montecito) and did not generalize. It now aborts iff the master catchment is a below-floor **fraction of the AOI's valid DEM area**: `assert_master_outlet_scale` ([pipeline.py:214](../src/pipeline.py#L214)) GateAborts when `master_km2` is non-finite/≤0, `valid_area_km2` ≤ 0, or `master_km2 / valid_area_km2 < MASTER_MIN_AOI_FRACTION` (0.05). Lower-only (a master ≈ AOI is a clean single-drainage crop). The reconstructed Montecito master is **44.7273 km²** = **26.5%** of its 168.93 km² valid AOI ([test_behavior_lock.py:94](../tests/core/test_behavior_lock.py#L94)) → well above the 5% floor, so it does not abort. `MASTER_KNOWN_KM2 = 39.19` survives only as a print-only reference in `validation/gate.py`; treat 44.7273 as truth. There is **no per-fire delineation-confidence surface** — the guard is a binary collapse detector (none until the P4 reference check).
...
The four `SUBBASIN_*` constants (A39) are frozen the same way, scoped to the **incised tier only** — never promotable to range-front scoring. Unlike the others, their provenance is explicitly **not result-blind**: the author had already seen South Fork / Trout / Montecito / Cooks Peak / Putah output at these settings before they were frozen, and the slope floor specifically was added after degenerate flat basins appeared in that output (decision A39). Carried here as an honest caveat on the evidence, not a license to re-tune.

### Auto-acquire constants (scene selection + dNBR creation)

The auto-acquire subsystem (§7a–b) has its own frozen fence, ratified by the auto-acquire pre-registration (2026-07-17). These govern **input data quality only** — dNBR stays A34-unvalidated for ranking no matter how clean the scenes are — and re-opening one re-opens that pre-registration.

| Parameter | Value | File:Line | Rationale |
|---|---|---|---|
| `PRE_WINDOW_DAYS` | 90 d | [scene_select.py:50](../autoacquire/scene_select.py#L50) | Pre-fire scene must fall within 90 d before ignition |
| `GREENUP_DEFAULT_DAYS` / `GREENUP_MAX_DAYS` | 90 / 180 d | [scene_select.py:51-52](../autoacquire/scene_select.py#L51-L52) | Post-fire green-up ceiling (default; operator hard max) |
| `TILE_CLOUD_MAX_PCT` | 80.0 % | [scene_select.py:54](../autoacquire/scene_select.py#L54) | Coarse whole-tile cloud pre-filter; never the decisive gate |
| `BOX_GATE_FLOOR` | 0.50 | [scene_select.py:58](../autoacquire/scene_select.py#L58) | Decisive gate: pre-∩-post valid fraction over the box; **derived** from `DNBR_NODATA_FAILLOUD_FRAC = 0.20` |
| `RUBRIC_GOOD_PAIR` / `RUBRIC_OK_PAIR` | 0.90 / 0.75 | [scene_select.py:63-64](../autoacquire/scene_select.py#L63-L64) | Scorecard bands on pair-valid fraction |
| `RUBRIC_GOOD_SCENE_CLOUD` / `RUBRIC_OK_SCENE_CLOUD` | 0.05 / 0.15 | [scene_select.py:65-66](../autoacquire/scene_select.py#L65-L66) | Scorecard bands on per-scene cloud-over-AOI |
| `S2_BAD_SCL` | `{0,1,3,6,8,9,10,11}` | [scene_select.py:72](../autoacquire/scene_select.py#L72) | Sentinel-2 SCL classes masked as bad (gate + creation) |
| `LANDSAT_QA_CLOUD_BITS` (+ fill bit 0) | `(1,2,3,4)` | [scene_select.py:79-80](../autoacquire/scene_select.py#L79-L80) | Landsat QA_PIXEL bad bits (dilated cloud, cirrus, cloud, shadow) |
| `S2_MIN_BASELINE` | 4.0 | [scene_select.py:75](../autoacquire/scene_select.py#L75) | Sentinel-2 processing baseline ≥ 04.00 (the −1000 SR offset). Moved verbatim from `dnbr_create` 2026-07-28 (A41): enforced at search in the selector, asserted per-member at creation as the backstop |
| S2 SR scaling | `(DN − 1000)/10000` | [dnbr_create.py:224-225](../autoacquire/dnbr_create.py#L224-L225) | Sentinel-2 L2A surface reflectance |
| Landsat SR scaling | `DN × 0.0000275 − 0.2` | [dnbr_create.py:63](../autoacquire/dnbr_create.py#L63) | Landsat C2 L2 surface reflectance (`LS_SR_SCALE`, `LS_SR_OFFSET`) |
| `DNBR_PHYS_CLIP` | 2.0 | [dnbr_create.py:66](../autoacquire/dnbr_create.py#L66) | Physical `|dNBR| ≤ 2` bound (NBR ∈ [−1, 1]) |

The S2/Landsat band designations, SR scalings, and the NBR/dNBR formulas are transcribed verbatim from agency documentation (ESA/Copernicus SentiWiki, USGS Collection-2 Level-2) and the validated working scripts — not reconstructed from memory.
